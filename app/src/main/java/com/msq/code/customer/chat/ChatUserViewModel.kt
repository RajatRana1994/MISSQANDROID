...
import com.msq.rest.entity.ChatUserListEntity
...
class ChatUserViewModel(
    ...
    val mChatUserListEntity = MutableLiveData<ChatUserListEntity>()
    ...
    fun lastChat() {
        if (context.isInternetAvailable()) {
            ...
            retrofitService.lastChat().subscribeOn(
                Schedulers.io()
            ).observeOn(AndroidSchedulers.mainThread())
                .subscribe({ response ->
                    ProgressDialogs.hideLoading()
                    if (response != null) {
                        mChatUserListEntity.value = response
                    } else {
