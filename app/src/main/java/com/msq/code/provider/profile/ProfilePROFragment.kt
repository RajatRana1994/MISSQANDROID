...
import com.msq.util.ext.toCamelCase
...
class ProfilePROFragment : BaseFragment(), View.OnClickListener {
    ...
    private fun observers() {
        ...
        viewModel.mServiceListEntity.observe(viewLifecycleOwner) {
            if (userData.services.isEmpty()) {
                msqServices.clear()
                msqServices.addAll(it.data.result)
                edtJob.setOnClickListener { v -> serviceDialog() }
            } else
                it.data.result.filter { it == userData.services[0] }.apply {
                    if (this.isNotEmpty()) {
                        edtJob.setText(this[0].name)
                        updateData()
                    } else {
                        msqServices.clear()
                        msqServices.addAll(it.data.result)
    ...
    val listPopupWindow by lazy { androidx.appcompat.widget.ListPopupWindow(requireActivity()) }
    val msqServices = arrayListOf<MSQService>()
    val mServiceAdapter by lazy { ServicePopAdapter(requireActivity(), msqServices) }

    fun serviceDialog() {
        listPopupWindow.setAdapter(mServiceAdapter)
        listPopupWindow.anchorView = edtJob
        listPopupWindow.isModal = false
        listPopupWindow.setBackgroundDrawable(
            ContextCompat.getDrawable(
                requireActivity(),
                R.drawable.round_bg_less_more
            )
        )
        listPopupWindow.width = androidx.appcompat.widget.ListPopupWindow.WRAP_CONTENT
        listPopupWindow.height = msqServices.size * 95
        listPopupWindow.animationStyle = android.R.style.Animation_Toast
        listPopupWindow.promptPosition - androidx.appcompat.widget.ListPopupWindow.POSITION_PROMPT_ABOVE
        listPopupWindow.verticalOffset = -24
        listPopupWindow.setOnItemClickListener { adapterView, view, i, l ->
            edtJob.text = msqServices[i].name.toCamelCase()
            selectedService = msqServices[i].id.toString()
            checkToUpdate()
...
//        btnSubmit.text =
//            if (selectedService.isEmpty() && city.isNotEmpty() && state.isNotEmpty() && postCode.isNotEmpty() && postCode.length < 5 && edtAddress.text.toString().trim()
//                    .equals(userData.address) && edtName.text.toString().trim()
//                    .equals(userData.name) && edtNumber.text.toString().trim()
...
//            }
        ...
        btnSubmit.text = if (edtName.text.toString().trim().equals(userData.name).not()
        ) getString(R.string.update_profile)
        else if (edtNumber.text.toString().trim().equals(userData.phone)
                .not()
        ) getString(R.string.update_profile)
        else if (edtAddress.text.toString().trim().equals(userData.address)
                .not()
        ) getString(R.string.update_profile)
        else if (selectedService.isEmpty().not()) getString(R.string.update_profile)
        else if (edtAddress.text.toString().trim()
                .equals(userData.address) && city.isNotEmpty() && state.isNotEmpty() && postCode.isNotEmpty() || (edtCity.isVisible
                    || edtState.isVisible
                    || edtZipcode.isVisible)
        ) {
            if (edtCity.text.toString().isNotEmpty() && edtState.text.toString()
                    .isNotEmpty() && edtZipcode.text.toString()
                    .isNotEmpty() && edtZipcode.text.toString().length > 4
            )
                getString(
                    R.string.update_profile
                )
    ...
    override fun onClick(v: View?) {
        when (v!!) {
            ...
            btnSubmit -> {
                if (btnSubmit.text.toString().equals(
                        getString(
                            R.string.update_profile
                        )
                    )
                    ...
                    postCode = edtZipcode.text.toString()//10.3384118","longitude":"123.9220882"
                    val json = JsonObject()
                    ...
                    if (selectedService.isNotEmpty()) {
                        json.addProperty("serviceIds", selectedService)
                    }
                    viewModel.updateProfile(json)
                } else if (btnSubmit.text.toString().equals("Logout")) {
                    LogoutAlert()
                } else if (btnSubmit.text.toString()
                        .equals(getString(R.string.feedback_to_artisans))
                ) {
                    showFragment(FragConst.EMP_SURVEY_PRO)
                }
            }
        }
    }
