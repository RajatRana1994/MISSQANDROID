...
public class CardValidator {
    ...
    public static int getCardIcon(@NotNull String brand) {
        if (brand.equalsIgnoreCase(Cards.MAESTRO.name)) return R.drawable.bt_ic_maestro;
        if (brand.equalsIgnoreCase(Cards.MASTERCARD.name)) return R.drawable.bt_ic_mastercard;
        if (brand.equalsIgnoreCase(Cards.DINERSCLUB.name)) return R.drawable.bt_ic_diners_club;
        if (brand.equalsIgnoreCase(Cards.JCB.name)) return R.drawable.bt_ic_jcb;
        if (brand.equalsIgnoreCase(Cards.UNIONPAY.name)) return R.drawable.bt_ic_unionpay;
        if (brand.equalsIgnoreCase(Cards.DISCOVER.name)) return R.drawable.bt_ic_discover;
        if (brand.equalsIgnoreCase(Cards.AMEX.name)) return R.drawable.bt_ic_amex;
        if (brand.equalsIgnoreCase(Cards.VISA.name)) return R.drawable.bt_ic_visa;
    ...
    public enum Cards {
        ...
        MAESTRO(R.drawable.bt_ic_maestro,"maestro", "^(5[06-9]|6[37])[0-9]{10,17}$", DEFAULT_CARD_FORMAT, new int[]{12,13,14,15,16,17,18,19}, new int[]{3}, true, true),
        MASTERCARD(R.drawable.bt_ic_mastercard,"mastercard", "^5[0-5][0-9]{14}$", DEFAULT_CARD_FORMAT, new int[]{16,17}, new int[]{3}, true, true), //check supported
        DINERSCLUB(R.drawable.bt_ic_diners_club,"dinersclub", "^3(?:0[0-5]|[68][0-9])?[0-9]{11}$", "(\\d{1,4})(\\d{1,6})?(\\d{1,4})?", new int[]{14}, new int[]{3}, true, true), //check supported
//        LASER(R.drawable.bt_ic_mastercard,"laser", "^(6304|6706|6709|6771)[0-9]{12,15}$", DEFAULT_CARD_FORMAT, new int[]{16,17,18,19}, new int[]{3}, true, false),
        JCB(R.drawable.bt_ic_jcb,"jcb", "^(?:2131|1800|35[0-9]{3})[0-9]{11}$", DEFAULT_CARD_FORMAT, new int[]{16}, new int[]{3}, true, true), //check supported
        UNIONPAY(R.drawable.bt_ic_unionpay,"unionpay", "^(62[0-9]{14,17})$", DEFAULT_CARD_FORMAT, new int[]{16,17,18,19}, new int[]{3}, false, false),
        DISCOVER(R.drawable.bt_ic_discover,"discover", "^6(?:011|5[0-9]{2})[0-9]{12}$", DEFAULT_CARD_FORMAT, new int[]{16}, new int[]{3}, true, true), //check supported
        AMEX(R.drawable.bt_ic_amex,"amex", "^3[47][0-9]{13}$", "^(\\d{1,4})(\\d{1,6})?(\\d{1,5})?$", new int[]{15}, new int[]{4}, true, true), //check supported
        VISA(R.drawable.bt_ic_visa,"visa", "^4[0-9]{12}(?:[0-9]{3})?$", DEFAULT_CARD_FORMAT, new int[]{13,16}, new int[]{3}, true, true); //check supported
        ...
        Cards(int icon,String name, String pattern, String format, int[] cardLength, int[] cvvLength, boolean luhn, boolean supported) {
            this.icon = icon;
            this.name = name;
    ...
    public static boolean validateExpiryDate(int month, int year) {
        ...
        return (curYear == year) ? curMonth <= month&&month<=12 : curYear < year&&month<=12;
    }
