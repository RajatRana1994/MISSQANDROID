...
class ImageUtility constructor(private val context: Context) {
    ...
    private val file: File
        get() = File(
            Environment.getExternalStorageDirectory().toString() + File.separator + "Instacuts"
                    + File.separator
        )
    //get file name
...
//        dialog.setContentView(R.layout.upload_photo_btmsheet)
...
//
//        val cameraIntent = dialog.findViewById<FrameLayout>(R.id.btnCapture)
//        val galleryIntent = dialog.findViewById<FrameLayout>(R.id.btnUpload)
//        dialog.findViewById<TextView>(R.id.tvTitle).text =
//            if (visibility) "Capture or upload" else "Capture"
//        dialog.findViewById<TextView>(R.id.tvPrivacyLink).setOnClickListener {
...
//            )
//        }
//        dialog.findViewById<TextView>(R.id.tvLiabilityLink).setOnClickListener {
//            activity.startActivity(
//                Intent(activity, WebActivity::class.java).putExtra(
//                    "title_extra",
//                    activity.getString(R.string.waiver_release_liab)
//                )
//            )
//        }
//        galleryIntent.onOffVisibility(visibility)
//        val cancelIntentBtn = dialog.findViewById<ImageView>(R.id.ivClose)
...
//    }
    ...
    fun CameraIntent(activity: Activity, cameraRequestCode: Int): Uri {
        val root = file
        root.mkdirs()
        val filename = uniqueImageFilename
        val sdImageMainDirectory = File(root, filename)
...
*/
    ...
    fun hasPermissionInManifest(context: Context, permissionName: String): Boolean {
        ...
        try {
            ...
            val declaredPermisisans = packageInfo.requestedPermissions
            if (declaredPermisisans != null && declaredPermisisans.size > 0) {
                for (p in declaredPermisisans) {
                    if (p == permissionName) {
                        return true
                    }
                }
